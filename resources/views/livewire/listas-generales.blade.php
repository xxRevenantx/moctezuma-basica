<div class="space-y-6">
    @php
        ...
        $esPersonalizadoresProfesores = $this->esPersonalizadoresProfesores();
    ...
    <section class="relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
        ...
        <div class="p-5 sm:p-6">
            ...
            @if ($tipo_descarga === 'formatos' && $opcion_descarga === 'personalizadores')
                <div class="mb-5 rounded-2xl border border-sky-200 bg-sky-50/60 p-4 dark:border-sky-900/50 dark:bg-sky-950/20">
                    <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-center">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <p class="text-sm font-black text-slate-900 dark:text-white">Tipo de personalizador</p>
                                <p class="mt-1 text-xs font-semibold leading-5 text-slate-500 dark:text-slate-400">
                                    Alumnos conserva el formato actual. Profesores habilita selección docente multinivel, fotografía de referencia y carga académica del ciclo.
                                </p>
                            </div>
                        </div>
                        ...
                        <flux:field>
                            ...
                            <flux:select id="audiencia_personalizador" wire:model.live="audiencia_personalizador">
                                <flux:select.option value="alumnos">Alumnos</flux:select.option>
                                <flux:select.option value="profesores">Profesores</flux:select.option>
                            </flux:select>
                        </flux:field>
                    </div>
                </div>
                ...
                <flux:field>
                    ...
                    @if (!$nivel_id)
                        <p class="mt-1 text-xs font-semibold text-violet-600 dark:text-violet-300">
                            {{ $esPersonalizadoresProfesores
                                ? '“Todos los niveles” permite buscar y seleccionar profesores de toda la institución.'
    ...
    @if ($modo_descarga === 'seleccionados' && $esPersonalizadoresProfesores)
        <section class="relative overflow-hidden rounded-3xl border border-sky-200 bg-white shadow-sm dark:border-sky-900/50 dark:bg-neutral-900">
            ...
            <div class="flex flex-col gap-4 border-b border-sky-200 bg-sky-50/60 p-5 dark:border-sky-900/50 dark:bg-sky-950/15 lg:flex-row lg:items-center lg:justify-between sm:p-6">
                <div class="flex items-start gap-3">
                    <span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-sky-100 text-sky-700 dark:bg-sky-950/60 dark:text-sky-300">
                        <flux:icon.academic-cap class="h-5 w-5" />
                    </span>
                    <div>
                        <h2 class="text-base font-black text-slate-900 dark:text-white">Selección manual de profesores</h2>
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                            La selección se conserva aunque cambies de nivel. Un profesor que trabaja en varios niveles genera un solo personalizador con todos sus niveles.
                        </p>
                    </div>
                </div>

                <div class="flex flex-wrap items-center gap-2">
                    <span class="inline-flex items-center rounded-full border border-sky-200 bg-white px-3 py-1.5 text-xs font-black text-sky-700 dark:border-sky-900/50 dark:bg-neutral-900 dark:text-sky-300">
                        {{ $this->totalProfesoresSeleccionados }} seleccionados
                    </span>
                    <button type="button" wire:click="seleccionarTodos" @disabled($this->totalProfesoresDisponibles === 0)
                        class="inline-flex items-center gap-2 rounded-xl bg-[#006492] px-3.5 py-2 text-xs font-black text-white transition hover:bg-[#00547a] disabled:cursor-not-allowed disabled:opacity-50">
                        <flux:icon.check class="h-4 w-4" />
                        Agregar visibles
                    </button>
                    <button type="button" wire:click="limpiarSeleccion" @disabled($this->totalProfesoresSeleccionados === 0)
            ...
            <div class="border-b border-sky-100 bg-white px-4 py-3 dark:border-sky-900/40 dark:bg-neutral-900 sm:px-5">
                <div class="flex flex-wrap items-center gap-2">
            ...
            <div class="grid min-h-[430px] grid-cols-1 xl:grid-cols-[minmax(0,1fr)_380px]">
                <div class="border-b border-slate-200 xl:border-b-0 xl:border-r dark:border-neutral-800">
                    <div class="p-4 sm:p-5">
                        <div class="relative">
                            ...
                            <input type="search" wire:model.live.debounce.350ms="buscar_profesor"
                                placeholder="Buscar por nombre, apellidos, CURP, RFC o correo..."
                                class="w-full rounded-2xl border border-slate-200 bg-white py-3 pl-11 pr-4 text-sm font-semibold text-slate-800 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-sky-400 focus:ring-4 focus:ring-sky-100 dark:border-neutral-700 dark:bg-neutral-900 dark:text-slate-100 dark:focus:border-sky-600 dark:focus:ring-sky-950/60">
                        </div>
                        <div class="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
                            <span>{{ $this->totalProfesoresDisponibles }} profesor(es) visibles con los filtros actuales.</span>
                            <span>Ciclo {{ $this->cicloSeleccionado?->nombre ?? 'actual' }}</span>
                        </div>
                    </div>

                    <div class="max-h-[540px] overflow-y-auto border-t border-slate-100 dark:border-neutral-800">
                        @forelse ($this->profesoresDisponibles as $profesor)
                            @php $incluido = in_array((int) $profesor->id, $this->idsProfesoresSeleccionadosValidos, true); @endphp
                            <label wire:key="lista-global-profesor-{{ $profesor->id }}"
                                class="flex cursor-pointer items-center gap-4 border-b border-slate-100 px-4 py-3 transition last:border-b-0 hover:bg-sky-50/70 dark:border-neutral-800 dark:hover:bg-sky-950/15 sm:px-5">
                                <input type="checkbox" value="{{ $profesor->id }}" wire:model.live="profesores_seleccionados"
                                    class="h-5 w-5 shrink-0 rounded border-slate-300 text-[#006492] shadow-sm focus:ring-[#006492] dark:border-neutral-700 dark:bg-neutral-950">

                                @if ($profesor->foto_url)
                                    <img src="{{ $profesor->foto_url }}" alt="{{ $this->nombreProfesor($profesor) }}"
                                        class="h-12 w-12 shrink-0 rounded-2xl border border-slate-200 object-cover shadow-sm dark:border-neutral-700">
                                @else
                                    <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-slate-100 text-xs font-black text-slate-600 dark:bg-neutral-800 dark:text-slate-300">
                                        {{ $profesor->iniciales }}
                                    </span>
                                @endif

                                <span class="min-w-0 flex-1">
                                    <span class="block truncate text-sm font-black text-slate-900 dark:text-white">{{ $this->nombreProfesor($profesor) }}</span>
                                    <span class="mt-1 block text-xs font-semibold text-slate-500 dark:text-slate-400">
                                        RFC: {{ $profesor->rfc ?: 'Sin RFC' }}
                                        @if ($profesor->correo)
                                            · {{ $profesor->correo }}
                                        @endif
                                    </span>
                                    <span class="mt-1.5 block text-xs font-bold text-sky-700 dark:text-sky-300">
                                        {{ $this->textoNivelesProfesor($profesor) }}
                                    </span>
                                ...
                                @if ($incluido)
                                    <span class="hidden rounded-full bg-[#006492] px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-white sm:inline-flex">Incluido</span>
                                @endif
                            </label>
                        @empty
                ...
                <aside class="bg-slate-50/70 p-4 dark:bg-neutral-950/30 sm:p-5">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            ...
                            <h3 class="mt-1 text-sm font-black text-slate-900 dark:text-white">Profesores seleccionados</h3>
                        </div>
                        <span class="rounded-full bg-slate-900 px-2.5 py-1 text-xs font-black text-white dark:bg-white dark:text-slate-900">{{ $this->totalProfesoresSeleccionados }}</span>
                    </div>

                    @if ($this->resumenSeleccionProfesoresPorNivel->isNotEmpty())
                        <div class="mt-4 flex flex-wrap gap-2">
                            @foreach ($this->resumenSeleccionProfesoresPorNivel as $resumen)
                    ...
                    <div class="mt-4 max-h-[455px] space-y-2 overflow-y-auto pr-1">
                        @forelse ($this->profesoresSeleccionadosDetalle as $profesor)
                            <div wire:key="lista-global-profesor-seleccionado-{{ $profesor->id }}"
                                class="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                                <div class="flex items-start gap-3">
                                    @if ($profesor->foto_url)
                                        <img src="{{ $profesor->foto_url }}" alt="{{ $this->nombreProfesor($profesor) }}"
                                            class="h-10 w-10 shrink-0 rounded-xl border border-slate-200 object-cover dark:border-neutral-700">
                                    @else
                                        <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-[11px] font-black text-slate-600 dark:bg-neutral-800 dark:text-slate-300">
                                            {{ $profesor->iniciales }}
                                        </span>
                                    @endif
                                    <div class="min-w-0 flex-1">
                                        <p class="truncate text-xs font-black text-slate-900 dark:text-white">{{ $this->nombreProfesor($profesor) }}</p>
                                        <p class="mt-1 text-[11px] font-bold leading-4 text-sky-700 dark:text-sky-300">{{ $this->textoNivelesProfesor($profesor) }}</p>
                                        <p class="mt-1 text-[11px] font-semibold leading-4 text-slate-500 dark:text-slate-400">RFC: {{ $profesor->rfc ?: 'Sin RFC' }}</p>
                                        <p class="mt-1 text-[11px] font-semibold leading-4 text-slate-500 dark:text-slate-400">{{ $this->textoCargaProfesor($profesor) }}</p>
                                    </div>
                                    <button type="button" wire:click="quitarProfesorSeleccionado({{ $profesor->id }})"
                        ...
                        @empty
                            <div class="rounded-2xl border border-dashed border-slate-300 bg-white/70 p-5 text-center dark:border-neutral-700 dark:bg-neutral-900/70">
                                <p class="text-xs font-black text-slate-700 dark:text-slate-200">Todavía no hay profesores seleccionados.</p>
                                <p class="mt-1 text-[11px] font-semibold text-slate-500 dark:text-slate-400">Marca profesores en la lista de la izquierda.</p>
                            </div>
                        @endforelse
                    </div>
                </aside>
