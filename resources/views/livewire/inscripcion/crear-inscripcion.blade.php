  <div class="space-y-6">
      ...
      <form wire:submit.prevent="guardar" x-data="{
          paso: 1,
          sucio: false,
          irPaso(numero) {
              this.paso = Math.min(5, Math.max(1, Number(numero) || 1));
              this.$nextTick(() => {
                  this.$refs.inicioFormulario?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      
                  if (this.paso === 4) {
                      setTimeout(() => {
                          window.dispatchEvent(new Event('resize'));
                          window.tinymce?.get('observaciones-inscripcion-crear')?.execCommand('mceAutoResize');
                      }, 120);
                  }
              });
          },
          ...
          x-on:input.capture="sucio = true" x-on:change.capture="sucio = true"
          x-on:inscripcion-ir-paso.window="irPaso($event.detail.paso)"
          x-on:inscripcion-ir-primer-error.window="irPaso(pasoPorCampo(($event.detail.campos || [])[0] || 'curp'))"
          x-on:inscripcion-guardada.window="sucio = false"
          x-on:beforeunload.window="if (sucio) { $event.preventDefault(); $event.returnValue = ''; }"
        ...
        2 => ['Asignación', 'Ingreso escolar'],
        3 => ['Tutor', 'Domicilio'],
        4 => ['Complementaria', 'Notas y fotografía'],
        5 => ['Revisión', 'Confirmar registro'],
    ] as $numeroPaso => [$tituloPaso, $subtituloPaso])
                              <button type="button" @click="irPaso({{ $numeroPaso }})"
                                  class="group relative rounded-2xl border px-4 py-3 text-left transition"
                                  :class="paso === {{ $numeroPaso }} ?
                                      ...
                                      (paso > {{ $numeroPaso }} ?
                                          ...
                                          'border-slate-200 bg-white/70 hover:border-slate-300 dark:border-neutral-700 dark:bg-neutral-900/60'
                                          )">
                                  <div class="flex items-center gap-3">
                                      <span
                                          ...
                                          :class="paso === {{ $numeroPaso }} ?
                                              ...
                                              (paso > {{ $numeroPaso }} ? 'bg-emerald-500 text-white' :
                                                  'bg-slate-100 text-slate-500 dark:bg-neutral-800 dark:text-slate-300')">
                                          <span x-show="paso <= {{ $numeroPaso }}">{{ $numeroPaso }}</span>
                                          <flux:icon.check x-show="paso > {{ $numeroPaso }}" class="h-4 w-4" />
                                      </span>
                                      <span class="min-w-0">
                                          <span
                                              class="block text-sm font-black text-slate-800 dark:text-white">{{ $tituloPaso }}</span>
                                          <span
                                              class="block truncate text-[11px] text-slate-500 dark:text-slate-400">{{ $subtituloPaso }}</span>
                                      </span>
                                  </div>
                              </button>
                          @endforeach
                  ...
                  @error('formulario')
                      <div
                          class="mb-5 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700 dark:border-rose-900/40 dark:bg-rose-950/30 dark:text-rose-300">
                          {{ $message }}
                      </div>
                  @enderror
                  ...
                  @if ($curpSuccess)
                      <div x-data="{ mostrar: true }" x-init="setTimeout(() => {
                          ...
                          <p class="font-semibold">Datos externos obtenidos</p>
                  ...
                  <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px] xl:items-start">
                      <div class="min-w-0 space-y-6">
                          {{-- DATOS PERSONALES --}}
                          <section x-cloak x-show="paso === 1" x-transition.opacity class="space-y-5">
                              <div class="flex items-center gap-3">
                                  <div
                              ...
                              <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
                                  <div class="xl:col-span-2">
                                      ...
                                      <div class="flex flex-col gap-2 sm:flex-row">
                                          <div class="min-w-0 flex-1">
                                              <flux:input wire:model.live.debounce.600ms="curp" maxlength="18"
                                                  autocomplete="off" spellcheck="false" class="uppercase"
                                          ...
                                          <flux:button type="button" variant="ghost" wire:click="consultarCurp"
                                              ...
                                              :disabled="!$puedeConsultarCurpExterna"
                                              class="cursor-pointer rounded-xl border border-sky-200 bg-sky-50 text-sky-700 disabled:cursor-not-allowed disabled:opacity-50 dark:border-sky-900/40 dark:bg-sky-950/30 dark:text-sky-300">
                                              <span wire:loading.remove wire:target="consultarCurp"
                                                  class="inline-flex items-center gap-2">
                                                  <flux:icon.magnifying-glass class="h-4 w-4" />
                                                  Consultar datos
                                              </span>
                                              <span wire:loading wire:target="consultarCurp"
                                                  class="inline-flex items-center gap-2">
                                                  <span
                                                      class="h-4 w-4 animate-spin rounded-full border-2 border-sky-200 border-t-sky-600"></span>
                                                  Consultando…
                                      ...
                                      <div wire:loading.delay wire:target="curp"
                                          class="mt-2 rounded-xl border border-sky-200 bg-sky-50 px-3 py-2 text-xs font-semibold text-sky-700 dark:border-sky-900/40 dark:bg-sky-950/30 dark:text-sky-300">
                                          Validando primero en la base de datos local…
                                      </div>

                                      <div @class([
                                          'mt-2 rounded-xl border px-3 py-2.5 text-xs font-semibold',
                                          'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/30 dark:text-emerald-300' =>
                                              $curpEstado === 'disponible',
                                          'border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-900/40 dark:bg-rose-950/30 dark:text-rose-300' => in_array(
                                              $curpEstado,
                                              ['encontrada', 'invalida', 'error']),
                                          'border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-300' =>
                                              $curpEstado === 'incompleta',
                                          'border-sky-200 bg-sky-50 text-sky-700 dark:border-sky-900/40 dark:bg-sky-950/30 dark:text-sky-300' => in_array(
                                              $curpEstado,
                                              ['inicial', 'consultando', 'valida']),
                                      ])>
                                          <div class="flex items-center gap-2">
                                              @if ($curpEstado === 'disponible')
                                                  <flux:icon.check-circle class="h-4 w-4 shrink-0" />
                                              @elseif (in_array($curpEstado, ['encontrada', 'invalida', 'error']))
                                                  <flux:icon.exclamation-triangle class="h-4 w-4 shrink-0" />
                                              @else
                                                  <flux:icon.information-circle class="h-4 w-4 shrink-0" />
                                              @endif
                                              <span>{{ $curpMensaje }}</span>
                                          </div>
                                      </div>
                                      ...
                                      @if ($curpAdvertencia)
                                          <div
                                              ...
                                              <p class="font-semibold">Consulta externa</p>
                                              <p class="mt-1">{{ $curpAdvertencia }}</p>
                                          </div>
                                      @endif

                                      @if (!empty($curpDiferencias))
                                          <div
                                              class="mt-2 rounded-xl border border-violet-200 bg-violet-50 px-4 py-3 text-sm text-violet-800 dark:border-violet-900/40 dark:bg-violet-950/30 dark:text-violet-200">
                                              <p class="font-black">Diferencias detectadas</p>
                                              <p class="mt-1 text-xs">Los datos que ya capturaste no fueron
                                                  sobrescritos.</p>
                                              <div class="mt-3 space-y-2">
                                                  @foreach ($curpDiferencias as $diferencia)
                                                      <div
                                                          class="rounded-lg bg-white/70 px-3 py-2 text-xs dark:bg-neutral-900/60">
                                                          <p class="font-bold">{{ $diferencia['campo'] }}</p>
                                                          <p class="mt-1"><span
                                                                  class="text-slate-500">Capturado:</span>
                                                              {{ $diferencia['capturado'] }}</p>
                                                          <p><span class="text-slate-500">Servicio:</span>
                                                              {{ $diferencia['servicio'] }}</p>
                                                      </div>
                                                  @endforeach
                                              </div>
                                          </div>
                                  ...
                                  @if ($alumnoExistente)
                                      <div
                                          class="xl:col-span-4 overflow-hidden rounded-2xl border border-rose-200 bg-gradient-to-br from-rose-50 via-white to-amber-50 shadow-sm dark:border-rose-900/40 dark:from-rose-950/20 dark:via-neutral-900 dark:to-amber-950/20">
                                          <div
                                              class="flex flex-col gap-4 p-4 lg:flex-row lg:items-center lg:justify-between">
                                              <div class="flex min-w-0 items-center gap-4">
                                                  @if ($alumnoExistente['foto_url'])
                                                      <img src="{{ $alumnoExistente['foto_url'] }}"
                                                          alt="Fotografía del alumno"
                                                          class="h-16 w-16 shrink-0 rounded-2xl object-cover ring-2 ring-white shadow dark:ring-neutral-800">
                                                  @else
                                                      <div
                                                          class="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-rose-100 text-lg font-black text-rose-700 dark:bg-rose-950/40 dark:text-rose-300">
                                                          {{ $alumnoExistente['iniciales'] }}
                                                      </div>
                                                  @endif
                                                  ...
                                                  <div class="min-w-0">
                                                      <div class="flex flex-wrap items-center gap-2">
                                                          <h3
                                                              class="truncate font-black text-slate-800 dark:text-white">
                                                              {{ $alumnoExistente['nombre_completo'] }}</h3>
                                                          <span
                                                              class="rounded-full bg-rose-100 px-2.5 py-1 text-[10px] font-black uppercase text-rose-700 dark:bg-rose-950/40 dark:text-rose-300">
                                                              {{ $alumnoExistente['estatus_etiqueta'] }}
                                                          </span>
                                                      </div>
                                                      <p class="mt-1 text-sm text-slate-600 dark:text-slate-300">
                                                          {{ $alumnoExistente['mensaje'] }}</p>
                                                      <div
                                                          class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
                                                          <span><strong>Matrícula:</strong>
                                                              {{ $alumnoExistente['matricula'] }}</span>
                                                          <span><strong>Ciclo:</strong>
                                                              {{ $alumnoExistente['ciclo'] ?: 'Sin ciclo' }}</span>
                                                          <span><strong>Nivel:</strong>
                                                              {{ $alumnoExistente['nivel'] ?: 'Sin nivel' }}</span>
                                                          <span><strong>Ubicación:</strong>
                                                              {{ $alumnoExistente['ubicacion'] ?: 'Sin grado/semestre' }}</span>
                                                          <span><strong>Grupo:</strong>
                                                              {{ $alumnoExistente['grupo'] ?: 'Sin grupo' }}</span>
                                                      </div>
                                                  </div>
                                              </div>

                                              <div class="flex shrink-0 flex-wrap gap-2">
                                                  @if ($alumnoExistente['url_expediente'])
                                                      <a href="{{ $alumnoExistente['url_expediente'] }}"
                                                          class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 dark:border-neutral-700 dark:bg-neutral-800 dark:text-slate-200">
                                                          <flux:icon.folder-open class="h-4 w-4" /> Expediente
                                                      </a>
                                                  @endif
                                                  @if ($alumnoExistente['accion_recomendada'] === 'reingreso' && $alumnoExistente['url_reingreso'])
                                                      <a href="{{ $alumnoExistente['url_reingreso'] }}"
                                                          class="inline-flex items-center gap-2 rounded-xl bg-amber-500 px-3 py-2 text-xs font-bold text-white hover:bg-amber-600">
                                                          <flux:icon.arrow-path class="h-4 w-4" /> Ir a reingreso
                                                      </a>
                                                  @elseif ($alumnoExistente['url_editar'])
                                                      <a href="{{ $alumnoExistente['url_editar'] }}"
                                                          class="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-bold text-white hover:bg-indigo-700">
                                                          <flux:icon.pencil-square class="h-4 w-4" /> Ver matrícula
                                                      </a>
                                                  @endif
                                                  <button type="button" wire:click="limpiarResultadoCurp(true)"
                                                      class="inline-flex items-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700 hover:bg-rose-100 dark:border-rose-900/40 dark:bg-rose-950/30 dark:text-rose-300">
                                                      <flux:icon.x-mark class="h-4 w-4" /> Limpiar
                                                  </button>
                                              </div>
                                          </div>
                                      </div>
                                  ...
                                  <div>
                                      ...
                                      <flux:select wire:model.live="ciclo_escolar_id">
                                          <flux:select.option value="">Selecciona un ciclo escolar
                                          </flux:select.option>
                                  ...
                                  <div>
                                      ...
                                      <flux:select wire:model.live="ciclo_id">
                                          <flux:select.option value="">Selecciona el momento de ingreso
                                          </flux:select.option>
                                  ...
                                  <div>
                                      ...
                                      <flux:select wire:model.live="tipo_ingreso">
                                          ...
                                          <flux:select.option value="traslado">Traslado de otra institución
                                          </flux:select.option>
                                          <flux:select.option value="captura_historica">Captura histórica
                                          </flux:select.option>
                                      </flux:select>
                                      @error('tipo_ingreso')
                                          <p class="mt-2 text-xs font-semibold text-rose-600">{{ $message }}</p>
                                  ...
                                  @if ($tipo_ingreso === 'captura_historica')
                                      <div class="md:col-span-2">
                                          ...
                                          <p class="mt-1 text-xs text-amber-600 dark:text-amber-300">
                                              La captura histórica requiere permiso de edición académica y quedará
                                              registrada como motivo de estatus.
                                          </p>
                                  ...
                                  <div>
                                      ...
                                      <flux:select wire:model="estado_inscripcion">
                                          ...
                                          <flux:select.option value="preinscrito">Preinscrito, pendiente de activación
                                          </flux:select.option>
                                      </flux:select>
                          ...
                          {{-- ASIGNACIÓN ESCOLAR --}}
                          <section x-cloak x-show="paso === 2" x-transition.opacity class="space-y-5">
                              <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                  <div class="flex items-center gap-3">
                                      ...
                                      <div>
                                          <h2 class="text-lg font-bold text-slate-800 dark:text-white">Asignación
                                              escolar</h2>
                                          <p class="text-sm text-slate-500 dark:text-slate-400">
                                              El sistema calcula la generación y solo muestra grupos compatibles con el
                                              ciclo seleccionado.
                                          </p>
                                      </div>
                                  </div>
                                  ...
                                  <span
                                      class="inline-flex w-fit items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/30 dark:text-emerald-300">
                              ...
                              <div class="relative grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
                                  <div wire:loading.flex
                                      wire:target="ciclo_escolar_id,ciclo_id,tipo_ingreso,nivel_id,grado_id,semestre_id"
                                      class="absolute inset-0 z-20 items-center justify-center rounded-2xl bg-white/75 backdrop-blur-sm dark:bg-neutral-900/75">
                                      <div
                                          class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-3 shadow-lg dark:border-neutral-700 dark:bg-neutral-900">
                                          <svg class="h-5 w-5 animate-spin text-violet-600" viewBox="0 0 24 24"
                                              fill="none">
                                          ...
                                          <span class="text-sm font-bold text-slate-700 dark:text-slate-200">Validando
                                              asignación...</span>
                                      </div>
                                  </div>
                                  ...
                                  <div>
                                      <div class="mb-1 flex items-center gap-2">
                                          ...
                                          <span
                                              class="inline-flex rounded-full border border-rose-200 bg-rose-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-rose-700 dark:border-rose-900/40 dark:bg-rose-950/30 dark:text-rose-300">Obligatorio</span>
                                      </div>
                                  ...
                                  @if (!$esBachillerato)
                                      <div>
                                          ...
                                          <flux:select wire:model.live="grado_id"
                                              ...
                                              <flux:select.option value="">Selecciona un grado
                                              </flux:select.option>
                                              @foreach ($grados as $grado)
                                                  <flux:select.option value="{{ $grado->id }}">
                                                      {{ $grado->nombre }}°</flux:select.option>
                                              @endforeach
                                          </flux:select>
                                          @error('grado_id')
                                              <p class="mt-2 text-xs font-semibold text-rose-600">{{ $message }}</p>
                                  ...
                                  @else
                                      <div>
                                          ...
                                          <flux:select wire:model.live="semestre_id"
                                              ...
                                              <flux:select.option value="">Selecciona un semestre
                                              </flux:select.option>
                                              @foreach ($semestres as $semestre)
                                                  <flux:select.option value="{{ $semestre->id }}">
                                                      {{ $semestre->numero }}° semestre ·
                                                      {{ $semestre->grado?->nombre }}° grado
                                                  </flux:select.option>
                                              @endforeach
                                          </flux:select>
                                          @if ($tipo_ingreso === 'nuevo_ingreso')
                                              <p class="mt-1 text-xs text-violet-600">Se calcula desde el momento de
                                                  ingreso.</p>
                                          @endif
                                          @error('semestre_id')
                                              <p class="mt-2 text-xs font-semibold text-rose-600">{{ $message }}</p>
                                  ...
                                  <div>
                                      <div class="mb-1 flex items-center gap-2">
                                          ...
                                          <span
                                              class="inline-flex rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/30 dark:text-emerald-300">Automática</span>
                                      </div>
                                      <flux:input
                                          value="{{ $generacionAutomaticaLabel ?: 'Se calculará automáticamente' }}"
                                          readonly disabled />
                                      @error('generacion_id')
                                          <p class="mt-2 text-xs font-semibold text-rose-600">{{ $message }}</p>
                                      @enderror
                                  ...
                                  <div>
                                      ...
                                      <flux:select wire:model.live="grupo_id"
                                          :disabled="!$generacion_id || empty($grupos)">
                              ...
                              @if ($asignacionAdvertencia)
                                  <div
                                      class="flex flex-col gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-200 sm:flex-row sm:items-center sm:justify-between">
                              ...
                              @if ($generacion_id && $grupo_id && !$asignacionAdvertencia)
                                  <div
                                      class="overflow-hidden rounded-2xl border border-emerald-200 bg-gradient-to-r from-emerald-50 via-white to-sky-50 dark:border-emerald-900/40 dark:from-emerald-950/30 dark:via-neutral-900 dark:to-sky-950/30">
                                      <div
                                          class="flex items-center gap-2 border-b border-emerald-100 px-4 py-3 text-sm font-black text-emerald-700 dark:border-emerald-900/40 dark:text-emerald-300">
                                      ...
                                      <div class="grid grid-cols-2 gap-4 p-4 text-sm md:grid-cols-3 xl:grid-cols-6">
                                          <div>
                                              ...
                                              <p class="font-bold">
                                                  {{ $cicloAsignacion?->inicio_anio }}-{{ $cicloAsignacion?->fin_anio }}
                                              </p>
                                          </div>
                                          <div>
                                              <p class="text-xs text-slate-500">Nivel</p>
                                              <p class="font-bold">{{ $nivelAsignacion?->nombre }}</p>
                                          </div>
                                          <div>
                                              <p class="text-xs text-slate-500">
                                                  {{ $esBachillerato ? 'Semestre' : 'Grado' }}</p>
                                              <p class="font-bold">
                                                  {{ $esBachillerato ? $semestreAsignacion?->numero . '°' : $gradoAsignacion?->nombre . '°' }}
                                              </p>
                                          </div>
                                          <div>
                                              <p class="text-xs text-slate-500">Generación</p>
                                              <p class="font-bold">{{ $generacionAutomaticaLabel }}</p>
                                          </div>
                                          <div>
                                              <p class="text-xs text-slate-500">Grupo</p>
                                              <p class="font-bold">{{ $grupoAsignacion['label'] ?? '—' }}</p>
                                          </div>
                                          <div>
                                              <p class="text-xs text-slate-500">Fecha de captura</p>
                          ...
                          <section x-cloak x-show="paso === 4" x-transition.opacity class="space-y-5">
                              <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                  <div class="flex items-center gap-3">
                                      <div
                                      ...
                                      <div>
                                          ...
                                          <p class="text-sm text-slate-500 dark:text-slate-400">
                                              Registra notas internas de la inscripción. Cada ciclo escolar conserva su
                                              propia observación.
                                          </p>
                                      </div>
                                  </div>
                                  ...
                                  @if ($ciclo_escolar_id)
                                      ...
                                      <span
                                          ...
                                          Ciclo
                                          {{ $cicloObservacion ? $cicloObservacion->inicio_anio . '-' . $cicloObservacion->fin_anio : 'seleccionado' }}
                                      </span>
                                  @endif
                              </div>
                              ...
                              <div
                                  ...
                                  <x-forms.tinymce-observaciones model="observaciones"
                                      id="observaciones-inscripcion-crear" :value="$observaciones" :height="260" />
                          ...
                          {{-- TUTOR Y DOMICILIO --}}
                          <section x-cloak x-show="paso === 3" x-transition.opacity class="space-y-5">
                              <div class="flex items-center gap-3">
                                  <div
                                      class="flex h-10 w-10 items-center justify-center rounded-2xl bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300">
                          ...
                          <section x-cloak x-show="paso === 4" x-transition.opacity class="space-y-5">
                              ...
                              <div x-data="{
                                  ...
                                  usarTemporal(event) {
                                      ...
                                      this.detalleArchivo = `${(file.size / 1024 / 1024).toFixed(2)} MB · ${file.type || 'imagen'}`;
                                  ...
                                  <div
                                      class="h-1.5 w-full bg-gradient-to-r from-sky-500 via-indigo-500 to-fuchsia-500">
                                  </div>
                                          ...
                                          <div class="flex flex-col justify-center">
                                              ...
                                              <div x-show="nombreArchivo" x-transition class="mt-4">
                                                  <div
                                                      ...
                                                      <flux:icon.check-circle class="h-4 w-4" />
                                                      <span class="min-w-0">
                                                          <span class="block truncate" x-text="nombreArchivo"></span>
                                                          <span class="block text-[11px] opacity-80"
                                                              x-text="detalleArchivo"></span>
                                                      </span>
                                                  </div>
                                              </div>
                          ...
                          <section x-cloak x-show="paso === 5" x-transition.opacity class="space-y-5">
                              <div class="flex items-center gap-3">
                                  <div
                                      class="flex h-10 w-10 items-center justify-center rounded-2xl bg-indigo-100 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300">
                                      <flux:icon.clipboard-document-check class="h-5 w-5" />
                                  </div>
                                  <div>
                                      <h2 class="text-lg font-bold text-slate-800 dark:text-white">Revisión y
                                          confirmación</h2>
                                      <p class="text-sm text-slate-500 dark:text-slate-400">Comprueba la identidad y la
                                          asignación antes de registrar.</p>
                                  </div>
                              </div>

                              <div class="grid gap-4 md:grid-cols-2">
                                  <div
                                      class="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 dark:border-neutral-700 dark:bg-neutral-800/60">
                                      <p class="text-[11px] font-black uppercase tracking-wider text-slate-400">Alumno
                                      </p>
                                      <p class="mt-2 font-black text-slate-800 dark:text-white">
                                          {{ $resumenInscripcion['nombre'] }}</p>
                                      <dl class="mt-3 space-y-2 text-sm">
                                          <div class="flex justify-between gap-3">
                                              <dt class="text-slate-500">CURP</dt>
                                              <dd class="font-semibold text-slate-700 dark:text-slate-200">
                                                  {{ $resumenInscripcion['curp'] }}</dd>
                                          </div>
                                          <div class="flex justify-between gap-3">
                                              <dt class="text-slate-500">Matrícula</dt>
                                              <dd class="font-semibold text-slate-700 dark:text-slate-200">
                                                  {{ $resumenInscripcion['matricula'] }}</dd>
                                          </div>
                                          <div class="flex justify-between gap-3">
                                              <dt class="text-slate-500">Estado inicial</dt>
                                              <dd class="font-semibold text-slate-700 dark:text-slate-200">
                                                  {{ $resumenInscripcion['estado'] }}</dd>
                                          </div>
                                      </dl>
                                  </div>
                                  ...
                                  <div
                                      ...
                                      <p class="text-[11px] font-black uppercase tracking-wider text-slate-400">
                                          Asignación escolar</p>
                                      <dl class="mt-3 space-y-2 text-sm">
                                          <div class="flex justify-between gap-3">
                                              <dt class="text-slate-500">Ciclo</dt>
                                              <dd class="font-semibold text-slate-700 dark:text-slate-200">
                                                  {{ $resumenInscripcion['ciclo'] }}</dd>
                                          </div>
                                          <div class="flex justify-between gap-3">
                                              <dt class="text-slate-500">Nivel</dt>
                                              <dd class="font-semibold text-slate-700 dark:text-slate-200">
                                                  {{ $resumenInscripcion['nivel'] }}</dd>
                                          </div>
                                          <div class="flex justify-between gap-3">
                                              <dt class="text-slate-500">Grado/Semestre</dt>
                                              <dd class="font-semibold text-slate-700 dark:text-slate-200">
                                                  {{ $resumenInscripcion['ubicacion'] }}</dd>
                                          </div>
                                          <div class="flex justify-between gap-3">
                                              <dt class="text-slate-500">Generación</dt>
                                              <dd class="font-semibold text-slate-700 dark:text-slate-200">
                                                  {{ $resumenInscripcion['generacion'] }}</dd>
                                          </div>
                                          <div class="flex justify-between gap-3">
                                              <dt class="text-slate-500">Grupo</dt>
                                              <dd class="font-semibold text-slate-700 dark:text-slate-200">
                                                  {{ $resumenInscripcion['grupo'] }}</dd>
                                          </div>
                                      </dl>
                                  </div>
                              </div>

                              @if ($resumenInscripcion['listo'])
                                  <div
                                      class="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/30 dark:text-emerald-300">
                                      La inscripción está lista para guardarse.
                                  </div>
                              @else
                                  <div
                                      class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-700 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-300">
                                      Aún faltan {{ $resumenInscripcion['pendientes'] }} datos obligatorios o
                                      validaciones.
                                  </div>
                              @endif
                          </section>
                      ...
                      <aside class="xl:sticky xl:top-24">
                          <div
                              class="overflow-hidden rounded-[24px] border border-slate-200 bg-white/90 shadow-lg shadow-slate-200/50 dark:border-neutral-700 dark:bg-neutral-900/90 dark:shadow-black/20">
                              <div class="h-1 bg-gradient-to-r from-sky-500 via-indigo-500 to-fuchsia-500"></div>
                              <div class="p-5">
                                  <div class="flex items-start justify-between gap-3">
                                      <div>
                                          <p
                                              class="text-[11px] font-black uppercase tracking-[0.18em] text-indigo-500">
                                              Resumen en vivo</p>
                                          <h3 class="mt-1 font-black text-slate-800 dark:text-white">
                                              {{ $resumenInscripcion['nombre'] }}</h3>
                                      </div>
                                      <span @class([
                                          'rounded-full px-2.5 py-1 text-[10px] font-black uppercase',
                                          'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' =>
                                              $resumenInscripcion['listo'],
                                          'bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300' => !$resumenInscripcion[
                                              'listo'
                                          ],
                                      ])>
                                          {{ $resumenInscripcion['listo'] ? 'Listo' : $resumenInscripcion['pendientes'] . ' pendientes' }}
                                      </span>
                                  </div>

                                  <dl class="mt-5 space-y-3 text-sm">
                                      @foreach ([
        'CURP' => $resumenInscripcion['curp'],
        'Matrícula' => $resumenInscripcion['matricula'],
        'Ciclo' => $resumenInscripcion['ciclo'],
        'Nivel' => $resumenInscripcion['nivel'],
        'Grado/Semestre' => $resumenInscripcion['ubicacion'],
        'Generación' => $resumenInscripcion['generacion'],
        'Grupo' => $resumenInscripcion['grupo'],
        'Estado' => $resumenInscripcion['estado'],
    ] as $etiqueta => $valor)
                                          <div
                                              class="flex items-start justify-between gap-3 border-b border-slate-100 pb-2 last:border-0 dark:border-neutral-800">
                                              <dt class="text-slate-500 dark:text-slate-400">{{ $etiqueta }}</dt>
                                              <dd
                                                  class="max-w-[170px] text-right font-bold text-slate-700 dark:text-slate-200">
                                                  {{ $valor }}</dd>
                                          </div>
                                      @endforeach
                                  </dl>
                                  ...
                                  <div @class([
                                      'mt-5 rounded-2xl border px-3 py-3 text-xs font-semibold',
                                      'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/30 dark:text-emerald-300' =>
                                          $curpEstado === 'disponible',
                                      'border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-900/40 dark:bg-rose-950/30 dark:text-rose-300' => in_array(
                                          $curpEstado,
                                      ...
                                      'border-sky-200 bg-sky-50 text-sky-700 dark:border-sky-900/40 dark:bg-sky-950/30 dark:text-sky-300' => !in_array(
                                          $curpEstado,
                                          ['disponible', 'encontrada', 'invalida', 'error']),
                                  ])>
                                      {{ $curpMensaje }}
                                  </div>
                              </div>
                  ...
                  <div
                      class="mt-8 flex flex-col gap-3 border-t border-slate-200 pt-6 dark:border-neutral-800 sm:flex-row sm:items-center sm:justify-between">
                      <div class="flex gap-2">
                          <flux:button type="button" variant="ghost" wire:click="cancelar" @click="sucio = false"
                              class="cursor-pointer rounded-2xl">
                              Cancelar
                          </flux:button>

                          <flux:button type="button" variant="ghost" x-show="paso > 1" @click="irPaso(paso - 1)"
                              class="cursor-pointer rounded-2xl border border-slate-200 dark:border-neutral-700">
                              <span class="inline-flex items-center gap-2">
                                  <flux:icon.arrow-left class="h-4 w-4" /> Anterior
                              </span>
                          </flux:button>
                      </div>

                      <div class="flex justify-end gap-2">
                          <flux:button type="button" variant="primary" x-show="paso < 5" @click="irPaso(paso + 1)"
                              class="cursor-pointer rounded-2xl">
                              <span class="inline-flex items-center gap-2">
                                  Continuar <flux:icon.arrow-right class="h-4 w-4" />
                              </span>
                          </flux:button>

                          <flux:button type="submit" variant="primary" x-show="paso === 5"
                              wire:loading.attr="disabled" wire:target="guardar,foto,curp"
                              :disabled="!$resumenInscripcion['listo'] || (bool) $alumnoExistente"
                              class="cursor-pointer rounded-2xl disabled:cursor-not-allowed disabled:opacity-50">
                              <span wire:loading.remove wire:target="guardar">{{ $textoBotonGuardar }}</span>
