<div x-data="{ show: false, loading: false }" x-cloak x-trap.noscroll="show" x-show="show"
    ...
    <div class="relative w-[92vw] sm:w-[88vw] md:w-[90vw] max-w-2xl mx-4 sm:mx-6
        ...
        role="dialog" aria-modal="true" aria-labelledby="titulo-modal-directivo" x-show="show"
        ...
        <div
            ...
            <div class="min-w-0">
                <h2 id="titulo-modal-directivo" class="text-xl sm:text-2xl font-bold text-neutral-900 dark:text-white">
                    Editar Directivo
                </h2>
                ...
                <p class="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
                    ...
                    @else
                        <span>Actualiza la información del directivo seleccionado.</span>
                    @endif
                </p>
            </div>
            ...
            <form wire:submit.prevent="actualizarDirectivo">
                <flux:field class="p-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <flux:input badge="Requerido" wire:model.blur="titulo" :label="__('Título')" type="text"
                            placeholder="Ej.: M.C., Lic., Dr., Mtro., Profr." autocomplete="off" />

                        <flux:input badge="Requerido" wire:model.blur="nombre" :label="__('Nombre')" type="text"
                            placeholder="Nombre del directivo" autocomplete="given-name" />

                        <flux:input badge="Requerido" wire:model.blur="apellido_paterno" :label="__('Apellido Paterno')"
                            type="text" placeholder="Apellido paterno" autocomplete="family-name" />

                        <flux:input badge="Opcional" wire:model.blur="apellido_materno" :label="__('Apellido Materno')"
                            type="text" placeholder="Apellido materno" autocomplete="additional-name" />

                        <flux:input badge="Opcional" wire:model.blur="curp" :label="__('CURP')" type="text"
                            placeholder="CURP del directivo" maxlength="18" autocomplete="off" class="uppercase" />

                        <flux:input badge="Opcional" wire:model.blur="rfc" :label="__('RFC')" type="text"
                            placeholder="RFC del directivo" maxlength="13" autocomplete="off" class="uppercase" />

                        <flux:input badge="Requerido" wire:model.blur="cargo" :label="__('Cargo')" type="text"
                            placeholder="Cargo" autocomplete="organization-title" />

                        <flux:input badge="Requerido" wire:model.blur="identificador" :label="__('Identificador')"
                            type="text" placeholder="Ej.: rector, directora-general, supervisor-primaria"
                            autocomplete="off" />

                        <flux:input badge="Opcional" wire:model.blur="zona_escolar" :label="__('Zona escolar')"
                            type="text" placeholder="Ej.: 030, 137, Zona 021" autocomplete="off" />

                        <flux:input badge="Opcional" wire:model.blur="telefono" :label="__('Teléfono')"
                            type="text" placeholder="Teléfono" autocomplete="tel" />

                        <flux:input badge="Opcional" wire:model.blur="correo" :label="__('Correo Electrónico')"
                            type="email" placeholder="correo@dominio.com" autocomplete="email" />

                        <flux:select badge="Requerido" wire:model.blur="genero" :label="__('Género')">
                    ...
                    <div
                        class="mt-4 flex items-center justify-between gap-4 rounded-xl border border-neutral-200 dark:border-neutral-700 p-3">
                        <span class="text-xs text-gray-500 dark:text-gray-400">
                            Define si el directivo estará activo en el sistema.
                        </span>

                        <flux:checkbox wire:model.live="status" :label="__('Status')" />
                    </div>
                    ...
                    <div
                        ...
                        <flux:button variant="primary" type="submit" class="w-full sm:w-auto cursor-pointer"
                            wire:loading.attr="disabled" wire:target="actualizarDirectivo">
                            <span wire:loading.remove wire:target="actualizarDirectivo">
                                Guardar
                            </span>

                            <span wire:loading wire:target="actualizarDirectivo">
                                Guardando...
                            </span>
                        </flux:button>
                    </div>
