                ...
                <div class="space-y-3">
                    ...
                    <div>
                        <h2 class="text-2xl font-black tracking-tight text-slate-950 dark:text-white sm:text-3xl">
                            {{ auth()->user()?->isProfessor() ? 'Mi horario docente' : 'Horario general del profesor' }}
                        </h2>

                        <p class="mt-2 max-w-4xl text-sm leading-6 text-slate-600 dark:text-zinc-400">
                            {{ auth()->user()?->isProfessor()
                                ? 'Consulta tus materias y talleres asignados. Puedes filtrar la vista y descargar tu horario en PDF.'
                                : 'Consulta todas las materias del docente en una sola tabla. Filtra por nivel, materia, grado, grupo o día sin que la pantalla pierda su posición.' }}
                        </p>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 gap-4 xl:grid-cols-7">
                ...
                @unless (auth()->user()?->isProfessor())
                    <div class="xl:col-span-2">
                        <label class="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-wide text-slate-500 dark:text-zinc-400">
                            <flux:icon.user class="h-4 w-4" /> Profesor
                        </label>
                ...
                @else
                    <div class="xl:col-span-2 rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3 dark:border-sky-900/50 dark:bg-sky-950/20">
                        <p class="text-[10px] font-black uppercase tracking-wider text-sky-600">Profesor autenticado</p>
                        <p class="mt-1 truncate text-sm font-black text-sky-950 dark:text-sky-100">{{ $profesorSeleccionado?->nombre_completo ?? auth()->user()->name }}</p>
                    </div>
                @endunless
