...
class CrearCicloEscolar extends Component
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'fin_anio' => ['required', 'integer', 'digits:4', 'min:2000', "max:{$max}", function ($attribute, $value, $fail): void {
    ...
    {
        if (filled($value) && is_numeric($value)) $this->fin_anio = (int) $value + 1;
    }

    public function guardar(): void
    {
        $data = $this->validate();
        if (CicloEscolar::query()->where('inicio_anio', $data['inicio_anio'])->where('fin_anio', $data['fin_anio'])->exists()) {
        ...
        $nuevo = DB::transaction(function () use ($data) {
            ...
            if ($marcarActual) {
                CicloEscolar::query()->where('es_actual', true)->lockForUpdate()->get()->each(function (CicloEscolar $actual) use ($data): void {
                    $actual->update([
                        'es_actual' => false,
        ...
        $this->dispatch('swal', [
            ...
            'text' => 'Crear o cambiar el ciclo no mueve alumnos. La promoción se realiza desde Generales por generación.',
            'position' => 'top-end',
        ]);
