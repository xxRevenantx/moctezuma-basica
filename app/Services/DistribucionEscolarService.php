...
class DistribucionEscolarService
{
            ...
            ->map(function (Collection $porGeneracion) use ($nivel, $escuela, $director): array {
                ...
                $filas = $porGeneracion
                    ...
                    ->map(function (Collection $grupo) use ($nivel, $escuela, $director, $generacion): array {
                        ...
                        $primero = $grupo->first();
                        $categorias = $grupo->countBy(
                            fn (InscripcionCiclo $registro) => $this->categoriaCiclo($registro)
                        );

                        $vigentes = $grupo
                            ->filter(fn (InscripcionCiclo $registro): bool => $this->esVigenteEnCiclo($registro))
                            ->values();
                        ...
                        return [
                            ...
                            // Hombres/Mujeres históricos se conservan por compatibilidad con Excel.
                            'hombres' => $grupo
                                ->filter(fn (InscripcionCiclo $r) => $r->inscripcion?->genero === 'H')
                                ->count(),
                            'mujeres' => $grupo
                                ->filter(fn (InscripcionCiclo $r) => $r->inscripcion?->genero === 'M')
                                ->count(),

                            // Nuevos conteos vigentes para pantalla, PDF y Word.
                            'hombres_vigentes' => $vigentes
                                ->filter(fn (InscripcionCiclo $r) => $r->inscripcion?->genero === 'H')
                                ->count(),
                            'mujeres_vigentes' => $vigentes
                                ->filter(fn (InscripcionCiclo $r) => $r->inscripcion?->genero === 'M')
                                ->count(),

                            'total_historico' => $grupo->unique('inscripcion_id')->count(),
                            'activos' => $vigentes->count(),
                            'no_vigentes' => max(0, $grupo->unique('inscripcion_id')->count() - $vigentes->count()),
            ...
            ->when(($filtros['estado'] ?? 'todos') !== 'todos', function (Builder $q) use ($filtros): void {
                match ($filtros['estado']) {
                    ...
                    'inactivo' => $q->whereIn(
                        ...
                        ['inactivo', 'no_reinscrito', 'pendiente_reinscripcion', 'no_iniciado']
                    ),
                    default => null,
                };
            });
    ...
    {
        $estado = $registro->resultado_final
        ...
        return match ($estado) {
            ...
            default => $this->esVigenteEnCiclo($registro) ? 'activo' : 'inactivo',
        };
    }
...
private function esVigenteEnCiclo(InscripcionCiclo $registro): bool
        ...
        private function totalesFilas(Collection $filas): array
    {
        return [
            ...
            'hombres_vigentes' => (int) $filas->sum('hombres_vigentes'),
            'mujeres_vigentes' => (int) $filas->sum('mujeres_vigentes'),
